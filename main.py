...
if __name__ == '__main__':
    ...
    for temp_str in gen_plain_text_list:
        temp_r, temp_rounds = d.encrypt(key, temp_str)
        for i in range(16):
            temp_hm = find_hm(rounds[i], temp_rounds[i])
            data_for_box_plot[i].append(temp_hm)
    print("Hamming Distance Round Wise for 5 Different Plain Texts(hm = 1):", data_for_box_plot)
    ...
    for i in range(1, 5+1):
        temp_list = generate_strings(text, i, 1)
        gen_plain_text_list.append(temp_list[0])
    data_for_box_plot = []
    for i in range(16):
        data_for_box_plot.append([])
    for temp_str in gen_plain_text_list:
        ...
        for i in range(16):
            temp_hm = find_hm(rounds[i], temp_rounds[i])
            data_for_box_plot[i].append(temp_hm)
    print("Hamming Distance Round Wise for 5 Different Hamming Distances:(hm = 1,2,3,4,5)", data_for_box_plot)
    fig = plt.figure("5 Different Hamming Distances")
    plt.xlabel("Rounds")
    plt.ylabel("Hamming Distance")
    plt.boxplot(data_for_box_plot)
    plt.show()
    ...
    for temp_key in gen_key_list:
        ...
        for i in range(16):
            temp_hm = find_hm(rounds[i], temp_rounds[i])
            data_for_box_plot[i].append(temp_hm)
    print("Hamming Distance Round Wise for 5 Different Secret Texts:", data_for_box_plot)
    fig = plt.figure("5 Different Secret Keys")
    plt.xlabel("Rounds")
    plt.ylabel("Hamming Distance")
    plt.boxplot(data_for_box_plot)
    plt.show()
